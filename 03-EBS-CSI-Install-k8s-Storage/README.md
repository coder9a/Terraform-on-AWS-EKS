...
- **Folder:** `EKS-Cluster-Terraform-manifests`
- Verify Terraform State Storage S3 Bucket in `v1-versions.tf` and AWS Mgmt Console
```t
...
- **Folder:** `EKS-Cluster-Terraform-manifests`
- Verify Terraform State Locking AWS DynamoDB Table in `v1-versions.tf` and AWS Mgmt Console
```t
  # Adding Backend as S3 for Remote State Storage
  backend "s3" {
    bucket = "terraform-eks-s3-bucket"
...
## Step-08: v1-versions.tf
- **Folder:** `EBS-Terraform-manifests`
```t
...
## Step-09: v2-remote-state-datasource.tf
- **Folder:** `EBS-Terraform-manifests`
```t
...
```
## Step-10: v3-01-generic-variables.tf
- **Folder:** `EBS-Terraform-manifests`
```t
...
business_divsion = "hr"
```
## Step-13: v4-01-ebs-csi-datasources.tf
- **Folder:** `EBS-Terraform-manifests`
```t
...
## Step-14: v4-02-ebs-csi-iam-policy-and-role.tf
- **Folder:** `EBS-Terraform-manifests`
```t
...
```
## Step-15: v4-03-ebs-csi-helm-provider.tf
- **Folder:** `EBS-Terraform-manifests`
```t
...
```
## Step-16: v4-04-ebs-csi-install-using-helm.tf
- **Folder:** `EBS-Terraform-manifests`
```t
