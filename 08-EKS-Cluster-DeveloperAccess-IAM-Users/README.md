...
- **File:** v08-01-iam-role-eksdeveloper.tf
```t
...
- **File:** v08-02-iam-group-and-user-eksdeveloper.tf
```t
...
- **File:** v08-03-k8s-clusterrole-clusterrolebinding.tf
```t
...
- **File:** v08-05-k8s-role-rolebinding.tf
```t
# Resource: k8s Role
...
```t
# Stop Bastion Host
Login to AWS Mgmt Console -> EC2 -> franciscan-prod-Bastion-Host -> Instance State -> Stop 
...
```t
# Set password for franciscan-prod-eksdeveloper1 user
aws iam create-login-profile --user-name franciscan-prod-eksdeveloper1 --password @EKSUser101 --no-password-reset-required

# Create Security Credentials for IAM User and make a note of them
aws iam create-access-key --user-name franciscan-prod-eksdeveloper1

# Sample Output
Kalyans-Mac-mini:01-ekscluster-terraform-manifests kalyanreddy$ aws iam create-access-key --user-name franciscan-prod-eksdeveloper1
{
    "AccessKey": {
        "UserName": "franciscan-prod-eksdeveloper1",
...
## Step-17: Configure franciscan-prod-eksdeveloper1 user AWS CLI Profile and Set it as Default Profile
```t
...
aws configure --profile franciscan-prod-eksdeveloper1
...
# Set default profile
export AWS_DEFAULT_PROFILE=franciscan-prod-eksdeveloper1

# Get current user configured in AWS CLI
aws sts get-caller-identity
Observation: Should see the user "franciscan-prod-eksdeveloper1" from franciscan-prod-eksdeveloper1 profile, refer below sample output
...
Kalyans-Mac-mini:01-ekscluster-terraform-manifests kalyanreddy$ aws sts get-caller-identity
{
    ...
    "Arn": "arn:aws:iam::180789647333:user/franciscan-prod-eksdeveloper1"
}
Kalyans-Mac-mini:01-ekscluster-terraform-manifests kalyanreddy$ 
```
## Step-18: Assume IAM Role and Configure kubectl and Access Kubernetes Objects which user franciscan-prod-eksdeveloper1 has access
```t
...
aws sts assume-role --role-arn "arn:aws:iam::$ACCOUNT_ID:role/franciscan-prod-eks-developer-role" --role-session-name eksdevsession101
...
Kalyans-Mac-mini:01-ekscluster-terraform-manifests kalyanreddy$ aws sts get-caller-identity
{
    ...
    "Account": "180789647333",
    "Arn": "arn:aws:sts::180789647333:assumed-role/franciscan-prod-eks-developer-role/eksdevsession101"
}
Kalyans-Mac-mini:01-ekscluster-terraform-manifests kalyanreddy$ 
...
## Step-19: Assume IAM Role and Configure kubectl and Access Kubernetes Objects which user franciscan-prod-eksdeveloper1 don't have access
```t
# Verify aws-auth configmap
kubectl -n kube-system get configmap aws-auth -o yaml
...
## Step-21: Deploy Sample App to Dev Namespace using IAM User franciscan-prod-eksdeveloper1
- Sample App manifests are created using `YAML` and we can deploy them using `kubectl`
```t
# Change Directory 
...
# Deploy kube-manifests (YAML Format) to Dev Namespace using franciscan-prod-eksdeveloper1 user
kubectl apply -f 03-app1-kube-manifests/
Observation: 
1. franciscan-prod-eksdeveloper1 has full access to Dev Namespace as per Role and RoleBinding we have defined in v08-05-k8s-role-rolebinding.tf
2. We should see a successful creation of Kubernetes Deployment and Services

# Verify Dev Namespace resources using franciscan-prod-eksdeveloper1 user
...
## Step-23: Deploy Sample App to Dev Namespace using IAM User franciscan-prod-eksdeveloper1 - Terraform Manifests
- Sample App manifests are created using `TERRAFORM LANGUAGE` and we can deploy them using `Terraform Commands`
```t
# Verify User
aws sts get-caller-identity
...
# Deploy Terraform Manifests of Sample App to Dev Namespace using franciscan-prod-eksdeveloper1 user
terraform init
...
## Step-24: Provide S3 and DynamoDB Full Access to franciscan-prod-eks-developer-role
- We need to provide S3 and DynamoDB Access to role `franciscan-prod-eks-developer-role` to deploy Apps to Dev Namespace using user `franciscan-prod-eksdeveloper1` provided if we are using `Remote State Datasource` as S3 Bucket and for `State Locking` if we are uing DynamoDB
- In AWS Mgmt Console, go to Services -> IAM -> Roles -> franciscan-prod-eks-developer-role
- Add below two policies
  - AmazonS3FullAccess
  - AmazonDynamoDBFullAccess
- We can also automate this by adding below code by updating in  
- **File:** `25-EKS-DeveloperAccess-IAM-Users/01-ekscluster-terraform-manifests/v08-01-iam-role-eksdeveloper.tf`
```t
...
# Verify current user configured in aws cli
aws sts get-caller-identity
Observation: It should switch back to current AWS_DEFAULT_PROFILE franciscan-prod-eksdeveloper1
...
## Step-26: Apply Changes for franciscan-prod-eks-developer-role
```t
# Change Directory 
...
## Step-27: Deploy Sample App to Dev Namespace using IAM User franciscan-prod-eksdeveloper1 - Terraform Manifests - After fixing S3 and DynamoDB Access
...
# Assume IAM Role
aws sts assume-role --role-arn "arn:aws:iam::<REPLACE-YOUR-ACCOUNT-ID>:role/<REPLACE-YOUR-ROLE-NAME>" --role-session-name eksadminsession201
aws sts assume-role --role-arn "arn:aws:iam::$ACCOUNT_ID:role/franciscan-prod-eks-developer-role" --role-session-name eksdevsession103
...
Kalyans-Mac-mini:01-ekscluster-terraform-manifests kalyanreddy$ aws sts get-caller-identity
{
    ...
    "Account": "180789647333",
    "Arn": "arn:aws:sts::180789647333:assumed-role/franciscan-prod-eks-developer-role/eksdevsession103"
}
Kalyans-Mac-mini:01-ekscluster-terraform-manifests kalyanreddy$ 
...
## Step-29: Login as franciscan-prod-eksdeveloper1 user AWS Mgmt Console and Switch Roles
...
Remove franciscan-prod-eksdeveloper1 creds

# Clean-Up AWS Config File
vi /Users/kalyanreddy/.aws/config 
Remove franciscan-prod-eksdeveloper1 profiles

# List Profiles - AWS CLI
aws configure list-profiles
```

## Step-23: Comment the Policies in v08-01-iam-role-eksdeveloper.tf
- **File:** v08-01-iam-role-eksdeveloper.tf
```t
/*
